// pgsd-sessiond/src/font.zig
//
// 8x16 VGA-style bitmap font for the login UI.
//
// This file is a verbatim copy of semadraw/src/apps/term/font.zig
// at commit 9e193d7, taken under semadraw's own license (which
// matches PGSDF's per the substrate-vs-distribution split in ADR
// 0001). Duplicated rather than imported because:
//
//   1. semadraw exposes its high-level "semadraw" module (App,
//      Encoder, etc.) as a build.zig.zon dependency, but the
//      term-specific font lives in semadraw/src/apps/term/ which
//      isn't part of the published module surface.
//
//   2. The font is purely data (compile-time bitmap + atlas
//      generator) with no runtime dependencies. Duplicating ~500
//      lines of static data is cheaper than promoting it to a
//      proper sub-module of semadraw.
//
//   3. The font is stable; the only churn is adding new box-drawing
//      glyphs, which neither term nor pgsd-sessiond will drive.
//      If divergence becomes a real problem, a future commit can
//      hoist the file into a shared utf-fonts/ package consumed
//      by both. Until then, duplication is the right trade-off.
//
// Modifications from the original: none. Keep this file byte-for-byte
// identical to the upstream so any future hoist into a shared package
// is a pure rename + path adjustment, no merge work.
