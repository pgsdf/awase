# Awase Architectural Discipline

Status: Stated, 2026-04-23

This document states the discipline that governs what code Awase depends on
and what code Awase writes. It is the principle behind decisions that were
...
> **Awase depends only on code written with Awase's guarantees in mind.
> Everything else is either replaced or explicitly accepted as a named
> platform-transport dependency.**

## Why

Awase's central commitments are **determinism and stability**. A recording
...
changes what Awase does. A kernel module that adds a background thread
changes Awase's timing. A userspace daemon that buffers an event changes
Awase's sequencing. None of these are malicious; they are the normal
lifecycle of code written for other purposes. But they break Awase.

The discipline says: **if we want Awase's guarantees to hold, every layer
...
argument. The deeper reason Awase owns its guarantee path is not that
external code might be wrong. It is that when external code has a gap
Awase needs closed, closing it is not under Awase's control.
...
When Awase needs a feature, a fix, or a behaviour that a third-party
component does not provide, Awase has only three options under a
dependency: persuade the maintainers to add it (subordinating Awase's
...
the feature (subordinating Awase's objectives to the dependency's
scope). All three put Awase's ability to meet its own objectives in
someone else's hands. Owning the component removes that: a gap
becomes a thing Awase closes, on Awase's schedule, against Awase's
...
not something Awase could need to change (the CPU, the kernel
boundary). Where Awase can foresee needing the component to do
...
whether Awase can afford to have its objectives gated by another
project's governance.

### Worked precedent: inputfs and gesture support

This is not a hypothesis. Awase has already encountered exactly this
situation and the ownership posture is what resolved it.

Awase needed gesture recognition (n-click, multi-touch) in the input
path. The external `hms(4)` HID pointing-device driver Awase would
...
were not added, and no action toward adding them was observed. Awase
...
Awase required was not going to be provided on any timeline Awase
controlled, and changing that was not within Awase's power.

Because Awase owned the input path through inputfs, this did not block
Awase. The gesture recogniser was written as Awase-owned code
(`semainput/libsemainput/libsemainput.zig`), shaped by Awase's needs
...
`semadraw/docs/PHASE_2_5_VERIFICATION_STATUS.md`). The capability Awase
needed exists, is owned, and is proven, specifically because Awase was
...
Awase has already paid for and proven, not betting on an untested
principle.
...
1. **Replace.** Write a Awase-owned equivalent. This is the path for
   everything inside the guarantee path. inputfs replaces evdev.
   drawfs replaces Xlib. audiofs and semasound replace OSS and
   PulseAudio. The replacement is shaped by Awase's needs, not by
...
2. **Accept as platform transport.** Some dependencies cannot be
   ...
   treated as the boundary Awase runs on, not as components Awase's
   guarantees extend through. Awase code does not rely on undocumented
   behaviour of accepted dependencies, and when an accepted
   dependency fails or changes, Awase's response is part of the design
   rather than a surprise.

3. **Remove.** Some external dependencies exist because they were
   convenient at the time Awase started and are not required by Awase's
   goals. When the discipline is applied, these disappear. The code
   that depends on them disappears with them.

Every external component in Awase falls into one of these three.
...
This list enumerates what Awase accepts as the platform it runs on. It
...
- Input hardware (through HID transport today; everything above
  ...
  `docs/AWASE_USB_HID_BOUNDARY.md` for the boundary contract)

**FreeBSD kernel**
- Scheduler, memory manager, VM subsystem
- VFS and the filesystems that hold Awase's data
- Signal handling, process model, IPC primitives that Awase uses
  directly (shared memory, sockets, kqueue)
- USB stack above the controller (for HID transport; see
  `docs/AWASE_USB_HID_BOUNDARY.md` for the eleven entry points
  inputfs depends on)

**Language and toolchain**
- Zig compiler and its code generation
- Zig standard library, with the caveat that Awase code at determinism
  boundaries verifies stdlib behaviour rather than assuming it; see
  `docs/AWASE_ZIG_STDLIB_BOUNDARY.md` for the boundary contract and the
...
Everything else in Awase is either written by the project or being
...
the HID path; AD-2 closed 2026-05-17 and no Awase code path uses
evdev.
...
paths are not Awase-written. The replacement path would be direct
GPU programming, which is the largest dependency-replacement
Awase could undertake.
...
**File persistence.** Awase runs on whatever filesystem the host
...
package manager, and `sysrebase`); Awase inherits ZFS as a runtime
dependency only when delivered as PGSD. See
`docs/AWASE_STORAGE_DEPENDENCY.md` for the full layering.
...
guarantee path, not every line of code Awase touches.

**It does not mean hostility to external projects.** FreeBSD, Zig,
and the various libraries Awase does not use are fine projects pursuing
their own goals. Awase's discipline is about Awase's guarantees, not
about those projects' quality.

**It does not mean avoiding standards.** Awase uses POSIX socket APIs,
...
implementation can change in ways that affect Awase's guarantees.

**It does not mean changes are free.** Each replacement is
substantial work. The discipline says Awase will do that work; it does
not say the work happens all at once. Pragmatic sequencing is part
of the discipline, not a departure from it.

**It does not mean Awase never ships.** At every moment, Awase should be
...
For policies of this kind, Awase accepts a small impurity in the
...
1. **New features do not introduce new guarantee-path dependencies
   ...
   whether Y is acceptable (with reasoning) or whether a Awase
   replacement for Y is in scope.

2. **Existing guarantee-path dependencies are named, not hidden.**
   Anywhere Awase currently depends on code outside this document's
...
3. **Replacements preserve guarantees through the transition.** No
   replacement leaves Awase non-functional for an extended period. The
...
4. **Accepted dependencies can be revisited.** The accepted list in
   ...
   appears, a bug that affected Awase's guarantees, a platform change
   that made replacement more tractable, a Awase feature that requires
   guarantees the dependency cannot provide.

5. **Occam's razor applies.** If a Awase component has grown complex
...
the explicit enumeration of what Awase depends on.

If a short reference is needed in commit messages or BACKLOG entries,
cite this document by path: `docs/AWASE_ARCHITECTURAL_DISCIPLINE.md`.
