 ...
 * surface list without acquiring/releasing the lock. Use this variant
 * from code paths that have already acquired s->lock for other
 * reasons (e.g. drawfs_find_session_for_surface_locked).
 */
struct drawfs_surface *drawfs_surface_lookup_locked(struct drawfs_session *s,
    uint32_t surface_id);
 ...
 *
 * Acquires s->lock to find the surface and check fast paths, releases
 * around vm_pager_allocate if a new VM object is needed (AD-18.2 —
 * vm_pager_allocate must not run with s->lock held), re-acquires to
 * install or yield to a concurrent installer. See implementation
 * comment for race-window analysis.
 */
