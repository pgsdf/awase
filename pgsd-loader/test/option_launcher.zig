// option_launcher: emulation-only harness for load-option forwarding
// (ADR 0003 criterion 5). Installed as the default boot application
...
pub fn main() uefi.Status {
    ...
    _ = con_out.outputString(L("option-launcher: starting pgsd-loader with options\r\n")) catch {};
    ...
    const target_path = device_path.createFileDevicePath(
        uefi.pool_allocator,
        std.unicode.utf8ToUtf16LeStringLiteral("\\EFI\\pgsd\\pgsd-loader.efi"),
    ) catch return .load_error;

    const child = bs.loadImage(false, uefi.handle, .{ .device_path = target_path }) catch {
        _ = con_out.outputString(L("option-launcher: LoadImage failed\r\n")) catch {};
        return .load_error;
    };
