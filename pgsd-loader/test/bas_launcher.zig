...
pub fn main() uefi.Status {
    ...
    _ = con_out.outputString(L("bas-launcher: starting pgsd-loader-bas\r\n --\r\n")) catch {};
    ...
    const target_path = device_path.createFileDevicePath(
        uefi.pool_allocator,
        std.unicode.utf8ToUtf16LeStringLiteral("\\EFI\\pgsd\\pgsd-loader-bas.efi"),
    ) catch return .load_error;

    const child = bs.loadImage(false, uefi.handle, .{ .device_path = target_path }) catch {
        _ = con_out.outputString(L("bas-launcher: LoadImage failed\r\n")) catch {};
        return .load_error;
    };
