...
pub const Font = struct {
    ...
    pub fn charToIndex(c: u21) ?u32 {
        ...
        // AD-48: NO-BREAK SPACE renders as a blank (the space
        // glyph), not the checkerboard fallback. FreeBSD's tree
        // indents continuation lines with NBSP; the operator's
        // hexdump showed e2 94 82 c2 a0 c2 a0 20 per line, and the
        // checkerboard columns in tree output were these NBSPs,
        // never the box-drawing characters.
        if (c == 0x00A0) {
            return 0; // space (FIRST_CHAR)
        }
    ...
    fn generateBoxGlyph(atlas: *[ATLAS_WIDTH * ATLAS_HEIGHT]u8, atlas_x: usize, atlas_y: usize, char_code: u21) void {
        ...
        for (0..GLYPH_HEIGHT) |row| {
            for (0..GLYPH_WIDTH) |col| {
                ...
                // AD-48 latent fix: assign directly. The previous
                // max-write conditional read atlas[idx] before any
                // write, and the atlas is declared undefined; every
                // non-segment pixel of every box glyph was a read of
                // undefined memory. Each box cell pixel is computed
                // exactly once here, so direct assignment fully
                // initializes the cell by construction.
                atlas[idx] = pixel;
            }
        }
    }
