...
backward so the display belongs to Awase from power-on; the loader will
live in `pgsd-boot/`, and Phase 0 (observation, recovery posture, and
the kernel-entry ABI documented from source) is complete as revised on
Apple firmware. **Source of truth for the PGSD kernel** (AD-57, drafted
...
are reproducible from the repository. Both are design-stage; see
`docs/design/` and the AD-56/AD-57 entries in `BACKLOG.md`.
...
├── pgsd-boot/       PGSD distribution layer: fresh Awase boot loader (AD-56; design only, no loader code yet)
