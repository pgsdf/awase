...
definition. **Boot-path ownership** (AD-56, ratified 2026-06-21)
replaces FreeBSD's `loader.efi` and its menu with a fresh Awase boot
loader, extending AD-4's "Awase owns the floor" backward so the display
belongs to Awase from power-on; Phase 0 (observation, recovery posture,
and the kernel-entry ABI documented from source) is complete as revised
on Apple firmware. Its Phase 3, the fresh loader itself, is now under
implementation as **AD-62** in `pgsd-loader/`: written fresh rather than
forked, it advances by bench-proven increments under a project
deployment architecture (project ADR 0001) and a kernel handoff contract
(`pgsd-loader/docs/KERNEL-HANDOFF.md`) derived from FreeBSD source and
verified against the pinned kernel. L0 (presence and chainload) is
closed; L3a.1 (the handoff contract) is complete; L3a.2 (the Boot
Artifact Store kernel path) is bench-proven on Apple hardware through
reading and verifying a published slot and loading the kernel ELF into
staging, with the metadata chain, firmware handoff, and first BAS-slot
boot the increments ahead. **Bootstrap recovery** (AD-59) is the recovery counterpart:
a loader-stage pipeline, living in `pgsd-boot/lua/`, that selects and
transfers to the correct boot environment without operator loader
knowledge, so a verified recovery environment is reachable, not merely
present. Its four-responsibility pipeline (Discover, Decide, Bind,
Transfer) is validated end to end on hardware; today it runs as a stock
loader extension and is written to migrate into the AD-56 loader, with the
loader-stage trigger and binding producers the remaining work. **Source of
truth for the PGSD kernel** (AD-57, drafted
2026-06-21) defines the canonical kernel as a pinned FreeBSD revision
plus an ordered set of project-maintained deltas (a recipe, not a
vendored source mirror), so a given kernel and its AD-56 measurements
are reproducible from the repository. AD-56's program is design-stage
except its Phase 3 loader, which is implemented and bench-advancing as
AD-62; AD-57 is design-stage; AD-59's pipeline is implemented and
hardware-validated with producer work remaining. See `docs/design/`,
`pgsd-loader/docs/`, and the AD-56, AD-57, AD-59, and AD-62 entries in
`BACKLOG.md`.
...
├── pgsd-loader/     PGSD distribution layer: the fresh Awase loader (AD-62 = AD-56 Phase 3; L0 closed, L3a kernel path bench-advancing)
├── pgsd-boot/       PGSD distribution layer: AD-59 recovery bootstrap pipeline in lua/, hardware-validated
├── s6/              Awase supervision tree, installed to /var/service/awase/ (AD-20)
...
`s6/awase/<name>/run` (installed to `/var/service/awase/` by install.sh),
