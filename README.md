...
Two distribution-level programs are in their design phase and own the
boot path and the kernel's definition. **Boot-path ownership** (AD-56,
ratified 2026-06-21) replaces FreeBSD's `loader.efi` and its menu with
a fresh Awase boot loader, extending AD-4's "Awase owns the floor"
backward so the display belongs to Awase from power-on; the fresh loader
will live in `pgsd-boot/`, and Phase 0 (observation, recovery posture, and
the kernel-entry ABI documented from source) is complete as revised on
Apple firmware. **Bootstrap recovery** (AD-59) is the recovery counterpart:
...
are reproducible from the repository. AD-56 and AD-57 are design-stage;
AD-59's pipeline is implemented and hardware-validated with producer work
remaining. See `docs/design/` and the AD-56, AD-57, and AD-59 entries in
`BACKLOG.md`.
...
├── pgsd-boot/       PGSD distribution layer: boot loader work (AD-56 fresh loader, design; AD-59 recovery bootstrap in lua/, hardware-validated)
